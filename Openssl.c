...
uint8 claa_ssl_req(st_tcp_buffer *pkg)
{
    BIGNUM* bne = NULL;
    int len = 0;
    int ret = 0;
    st_tcp_buffer* pkg = NULL;
    uint32 remain_len = 0;
	...
	claa_set_tcp_buffer(&pkg);
    remain_len = RSA_N_LEN_BYTE + RSA_E_LEN_BYTE + 1; 
    pkg->data[pkg->len] = ENCRYPTREQ;
    pkg->len++;
    do 
	{
        byte = remain_len % 128;
        ...
        pkg->data[pkg->len++] = byte;
        remain_count++;
    }while(remain_len > 0 && remain_count < 5);
	if (remain_count >= 5)
	{
		return 0;
	}
    ...
    /* write e */
    memset(pkg->data + pkg->len, 0, RSA_E_LEN_BYTE);
    ...
    memcpy(pkg->data + pkg->len + RSA_E_LEN_BYTE - len, tmp, len);
    pkg->len += RSA_E_LEN_BYTE;

    /* write n */
    len = BN_bn2bin(g_rsa->n, pkg->data + pkg->len);
    if (len != RSA_N_LEN_BYTE)
	{
		claa_set_tcp_buffer(&pkg);
		return 0;
    }
    pkg->len += RSA_N_LEN_BYTE;
